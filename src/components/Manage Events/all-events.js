...
function Allevents() {
  ...
  const allEvent = (page) => {
    fetch(
      `https://nearbyplaceadminpanner.onrender.com/api/v1/allevents?page=${page}`,
      {
        method: "GET",
        credentials: "include",
      }
  ...
  useEffect(() => {
    const searchEventData = async () => {
      const response = await fetch(
        `https://nearbyplaceadminpanner.onrender.com/api/v1/allevents`,
        { credentials: "include" }
      );
  ...
  const inputSearchEvent = (e) => {
    if (e.target.value == "") {
      ...
      const filterres = searcheventsave.filter((item) =>
  ...
  const updateStatusDetail = async (id, status) => {
    ...
    await fetch(
      `https://nearbyplaceadminpanner.onrender.com/api/v1/events/${id}`,
      {
        method: "PUT",
        credentials: "include",
        body: JSON.stringify({
                              ...
                              ? eventData.map((items, index) => (
                                  <tr key={index}>
                                    ...
                                    <td className="click-color">
                                      ...
                                      To{" "}
                                      {dateFormat(items.dateend, "dd-mm-yyyy")}
                                    </td>
                                    <td>
                                      {items.status === true ? (
                                    ...
                                    <td className="click-color">
                                      <Link
                                        to={"/event-participants/" + items._id}
                                      >
                                    ...
                                    <td className="action-btn">
                                      ...
                                      <Link to={"/event-edit/" + items._id}>
                                        <i class="fa fa-pencil-square-o" />
                                      </Link>
                                      <a>
                                        <Button
                                          onClick={(e) =>
                                            toggleEventModel(items._id, e)
                                          }
                                        >
